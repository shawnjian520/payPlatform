...
//初始化区域图的对象
	...
	if(property.haveHead){
		...
		if(property.headLabel){
			tmp+="<label>"+property.initLabelText+"</label><span></span>";
		}
		for(var x=0;x<property.headBtns.length;++x){
			tmp+="<a href='javascript:void(0)' class='GooFlow_head_btn'><i class='ico_"+property.headBtns[x]+"' ";
			if(property.headBtns[x]=='new'){
				tmp += " title='新增'";
			}else if(property.headBtns[x]=='cancel'){
				tmp += " title='取消'";
			}else if(property.headBtns[x]=='save'){
				tmp += " title='保存'";
			}else if(property.headBtns[x]=='close'){
				tmp += " title='删除'";
			}
			tmp+="></i></a>";
		}
		...
		this.$head.on("click",{inthis:this},function(e){
			...
			switch($(tar).attr("class")){
				case "ico_new":		//if(This.onBtnNewClick!=null)	This.onBtnNewClick();
					subPagepro.addData();
					break;
				case "ico_cancel":		
					subPagepro.closewrt();
				...
				case "ico_save":	//if(This.onBtnSaveClick!=null)	This.onBtnSaveClick();
					subPagepro.save();
					break;
	...
	if(property.haveTool){
		...
		//未加代码：加入绘图工具按钮
		this.$tool.append(
      "<a href='javascript:void(0)' type='cursor' class='GooFlow_tool_btndown' id='"+this.$id+"_btn_cursor' title='选中'><i class='ico_cursor'/></a>"
     +"<a href='javascript:void(0)' type='direct' class='GooFlow_tool_btn' id='"+this.$id+"_btn_direct' title='画线'><i class='ico_direct'/></a>"
    );
...
GooFlow.prototype={
	...
	initDraw:function(id,width,height){
		...
		$(this.$draw).delegate(tmpClk,"click",{inthis:this},function(e){
			//subPagepro.editData_line(this.id);
...
//			});
	...
	addNode:function(id,json){
		...
		if(json.type.indexOf(" round")<0){
			...
			this.$nodeDom[id]=$("<div class='GooFlow_item"+mark+" " +json.jdzt+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='1' style='width:"+(json.width-2)+"px;height:"+(json.height-2)+"px;'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td><td>"+json.name+"</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		}
		else{
			json.width=24;json.height=24;
			//this.$nodeDom[id]=$("<div class='GooFlow_item item_round"+mark+" " +json.jdzt+"' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
			this.$nodeDom[id]=$("<div class='GooFlow_item' id='"+id+"' style='top:"+json.top+"px;left:"+json.left+"px'><table cellspacing='0'><tr><td class='ico'><i class='ico_"+json.type+"'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>"+json.name+"</div></div>");
		}
      ...
      if(mark&&GooFlow.prototype.color.mark){
       ...
       this.$nodeDom[id].addClass("item_mix");
		...
		if(id.split('_').length>2){
			subPagepro.editnode(json.left,json.top,json.type);
		}
...
//		this.switchToolBtn("cursor");
	...
	moveNode:function(id,left,top){
		...
		subPagepro.move(id.split('_')[1],left,top);
	},
	//设置结点/连线/分组区域的文字信息
...
///////////以下为有关画线的方法
	...
	addLine:function(id,json){
		...
		if(id.split('_').length>2){
			subPagepro.saveline(json.from.split('_')[1],json.to.split('_')[1]);
		}
	},
