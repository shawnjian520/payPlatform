...
/**
 * tool 工具对象
 * @type {{hideMenu: null, choseLeftStageMenu: string, choseTopStageMenu: string, leftStageMenuList: Array, res_form: null, configDebug: boolean, userloginid: null, usertype: null, waitForResponseStatusBtn: null, procee: string, exportData: tool.exportData, exportSuccess_time: number, exportSuccess_c: number, exportSuccess: tool.exportSuccess, previewImage: tool.previewImage, previewImageUrl: tool.previewImageUrl, report: tool.report, report_s: tool.report_s, report_all: tool.report_all, report_open: tool.report_open, report_open_input: tool.report_open_input}}
 */
var tool = {
    hideMenu: null,
    ...
    exportData: function (par, path, filep) {
        ...
        var jsp = par.substring(0, par.indexOf('.jsp') + 4);
        var tj = '';
        if (par.indexOf('?') != -1) {
