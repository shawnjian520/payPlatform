...
<script type="text/javascript">
    ...
    if (cookielang == "en") {
        yck = 'Already view';
        wck = 'Not viewed';
        nochoosetitle = 'Please select the document you want to Send!';
    } else {
        yck = '已查看';
        wck = '未查看';
        nochoosetitle = '请选择要发送的公文!';
    }
    ...
    function send(el) {
        ...
        if (slength >= 1) {
            ...
            $.post(tool.initParams('<%=basePath%>/s/system/sysDoc/send'), {'wid': id}, function (data) {
                if (data == 'SUCCESS') {
                    tool.openview(el, '<%=basePath%>s/system/sysDoc/list', '已发公文');
                } else {
            ...
            <tr id="tr_${data.wid}">
                ...
                <td id="td${data.wid}">
                    <a style="cursor: pointer;"
                       onclick="tool.openview(this,'<%=basePath%>s/system/sysDoc/input/${data.wid}/2')">${data.xxbt}</a>
                </td>
                <td>
                    <fmt:formatDate value="${data.createtime }" pattern="yyyy-MM-dd HH:mm" var="createtime"/>
                        ${createtime }
                </td>
                <td><a style="cursor: pointer;" onclick="ckjsr('${data.wid}')" data-localize="frame_tzgl.xtz_jsbm">查看接收部门</a>
                </td>
            </tr>
