...
<div class="row">
    <div class="col-xs-12">
    	...
    	<div class="col-xxxl-3 col-md-4 col-sm-6 col-xs-12 nopadding">
            <div class="form-group">
                <label title="订单号" class="col-sm-3 control-label ell nopadding"> 订单号：</label>
                <div class="col-sm-9">
                    <input type="text" id="payOrderNo" name="payOrderNo" class="form-control"/>
                </div>
            </div>
        </div>
        <div class="col-xxxl-3 col-md-4 col-sm-6 col-xs-12 nopadding">
...
<div class="row mt5">
    <div class="col-md-12">
        <a class="btn btn-sm btn-success" onclick="page.refundMon(this)">
            退款<i class=""></i>
        </a>
...
<div class="dataTables_wrapper form-inline">
    <table class="table table-striped table-hover table-bordered table-order">
        ...
        <tr>
            ...
            <th data-order-name="">交易流水号</th>
            <th data-order-name="">支付金额(元)</th>
            <th data-order-name="">退款额度(元)</th>
            <th data-order-name="">支付类型</th>
            <th data-order-name="">支付描述</th>
            <th data-order-name="">支付状态</th>
            <th data-order-name="">支付时间</th>
            <th data-order-name="">支付申请时间</th>
            <th data-order-name="">支付过期时间</th>
        </tr>
        </thead>
        <tbody>
        <c:forEach items="${resultData}" var="obj" varStatus="sta">
            <tr id="tr_${obj.id}">
                ...
                <td><a onclick="tool.editDataById(this,'${obj.id}')" href="javascript:void(0)">${obj.payOrderNo}</a></td>
                ...
                <td><c:if test="${obj.status==0}">创建支付订单</c:if>
                	...
                	<c:if test="${obj.status==2}">支付成功</c:if>
...
<script>
	...
	page={
			doSearch:function(el){
	            ...
	            if (!isBlank(ids)) {
				 ...
				 tool.ajax({
		                type: 'post',
		                data: {"id":id},
		                url: basePath + '/s/biz/my-pay-order/getFlag',
			...
			} ,refundMon:function(el){
				var form_element = $(el).parents('.form-horizontal');
			    var id = tool.getChoseData(form_element);
			    if (!isBlank(id)) {
	                if (id.split(",").length > 1) {
	                	tool.alertWarning('不支持批量退款,请勾选1条数据!', '请在下方列表中勾选一条要退款的数据');
	                	return;
	                } 
	            } else {
	            	  tool.alertWarning('请选择要退款的数据!', '请在下方列表中勾选一条要退款的数据');
	            	  return;
	            }
				swal({
					  title: "请输入退款原因!",
					...
					function(reason){
			            ...
			            tool.ajax({
				                ...
				                url: basePath + '/s/biz/my-pay-order/refundMony',
				                success: function (data) {
						            /* tool.removeWaitForResponseStatus(); */
						            alert("退款成功！");
				                }
